...
function doGet(){
   ...
   //database lana ha database sa sb
    $dbconnect=mysqli_connect("localhost","root","","student1");
    $query=mysqli_query($dbconnect,"select * from `employee`".$where);
    while($data=mysqli_fetch_assoc($query)){
$response[]=array("emp_id"=>$data['id'],"emp_name"=>$data['emp_name'], "image"=>$data['image']);
    }
    return $response;
...
function doPost(){
    ...
    if($_POST){
      ...
      $dbconnect=mysqli_connect("localhost","root","","student1");
$query=mysqli_query($dbconnect,"INSERT INTO `employee`(`emp_name`,`emp_status`, `image`) VALUES ('".$_POST['emp_name']."','".$_POST['emp_status']."', '".$image."')");
    if($query==true){
